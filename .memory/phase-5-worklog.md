...
**Problem**: `raqam/core` was at ~2027 bytes gzipped, 27 bytes over the 2000-byte limit in `.size-limit.json`.
...
**Problem 1**: The original tsup config applied `banner: { js: '"use client";' }` to ALL entries (core, react, index) from a single config block. The `raqam/core` (= `raqam/server`) entry should NOT have `"use client"` — it's a server-safe bundle.
...
| `docs/src/content/docs/guides/nextjs.mdx`               | Client components, `raqam/server` for RSC, Edge Runtime, locale plugin pattern                    |
